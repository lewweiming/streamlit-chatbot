...
class X2FrameworkItem extends Model
{
    ...
    // {
    //     $result = parent::$db->table('x2_framework_items')->count('id', 'count')->get();
    ...
    {
        $result = parent::$db->table('x2_framework_items')->where($column, $value)->get();
    ...
    {
        $results = parent::$db->table('x2_framework_items')->where($column, $value)->getAll();
